...
class Festival:
    '''
    This class represents all the other festival page functions
    '''

    def __init__(self, db_path):
        '''
        Constructor Method For Class Festival
    ...
    def plot_current_month_daily_spend(self):
        '''
        This method returns current month daily amount spend on 'Festival Expenditure' category values 
        to plot line chart

        :return: returns current month daily amount spend on 'Festival Expenditure' category values
        :rtype: list
        '''
        ...
        for i in range(1, day+1):
            try:
                cursor.execute(query, (month, i, 'Festival Expenditure'))
    ...
    def get_latest_ten_spends(self):
        '''
        This method returns latest 10 amount spent transactions in 'Festival Expenditure' category

        :return: returns latest 10 amount spent transactions in 'Festival Expenditure' category
        :rtype: list
        '''

        cursor = self.__connection.cursor()
        query = "SELECT * FROM amount_spend as a WHERE a.'S No' > 0 and a.'Category' in ('Festival Expenditure') ORDER BY a.'S No' DESC  LIMIT 10;"
        try:
...
if __name__ == "__main__":
    festival = Festival(db_path="/Users/srinivas/Desktop/M_Monitor/main_monitor.db")
    print(festival.plot_current_month_daily_spend())
    print(festival.get_latest_ten_spends())
    print(festival.db_path)
    print(festival.connection)