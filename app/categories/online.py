...
class OnlineShopping:
    '''
    This class represents all the other online shopping page functions
    '''

    def __init__(self, db_path):
        '''
        Constructor Method For Class OnlineShopping
    ...
    def plot_current_month_daily_spend(self):
        '''
        This method returns current month daily amount spend on 'Online Shopping' category values 
        to plot line chart

        :return: returns current month daily amount spend on 'Online Shopping' category values
        :rtype: list
        '''
        ...
        for i in range(1, day+1):
            try:
                cursor.execute(query, (month, i, 'Online Shopping'))
    ...
    def get_latest_ten_spends(self):
        '''
        This method returns latest 10 amount spent transactions in 'Online Shopping' category

        :return: returns latest 10 amount spent transactions in 'Online Shopping' category
        :rtype: list
        '''

        cursor = self.__connection.cursor()
        query = "SELECT * FROM amount_spend as a WHERE a.'S No' > 0 and a.'Category' in ('Online Shopping') ORDER BY a.'S No' DESC  LIMIT 10;"
        try:
...
if __name__ == "__main__":
    shopping = OnlineShopping(db_path="/Users/srinivas/Desktop/M_Monitor/main_monitor.db")
    print(shopping.plot_current_month_daily_spend())
    print(shopping.get_latest_ten_spends())
    print(shopping.db_path)
    print(shopping.connection)