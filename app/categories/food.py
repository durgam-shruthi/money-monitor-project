...
class Food:
    '''
    This class represents all the other food page functions
    '''

    def __init__(self, db_path):
        '''
        Constructor Method For Class Food
    ...
    def plot_current_month_daily_spend(self):
        '''
        This method returns current month daily amount spend on 'Food' category values 
        to plot line chart

        :return: returns current month daily amount spend on 'Food' category values
        :rtype: list
        '''
        ...
        for i in range(1, day+1):
            try:
                cursor.execute(query, (month, i, 'Food'))
    ...
    def get_latest_ten_spends(self):
        '''
        This method returns latest 10 amount spent transactions in 'Food' category

        :return: returns latest 10 amount spent transactions in 'Food' category
        :rtype: list
        '''

        cursor = self.__connection.cursor()
        query = "SELECT * FROM amount_spend as a WHERE a.'S No' > 0 and a.'Category' in ('Food') ORDER BY a.'S No' DESC  LIMIT 10;"
        try:
...
if __name__ == "__main__":
    food = Food(db_path="/Users/srinivas/Desktop/M_Monitor/main_monitor.db")
    print(food.plot_current_month_daily_spend())
    print(food.get_latest_ten_spends())
    print(food.db_path)
    print(food.connection)